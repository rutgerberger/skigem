// --- trip-planner/ChaletHunters.tsx ---
"use client";
...
export default function ChaletHunters() {
  ...
  // If we don't have budget/guests in URL, show the config form first
  const [isConfiguring, setIsConfiguring] = useState(!budgetParam || !guestsParam);
  
  // Config Form State
  const [configForm, setConfigForm] = useState({
    budget: budgetParam || "150",
    guests: guestsParam || "4",
  ...
  // Fetch DB resorts for mapping IDs
  useEffect(() => {
  ...
  const handleDeployHunters = (e: React.FormEvent) => {
    e.preventDefault();
    const params = new URLSearchParams(searchParams.toString());
    params.set("budget", configForm.budget);
    params.set("guests", configForm.guests);
    params.set("requirements", configForm.requirements);
    
    // Update URL, which will trigger the hunt effect below
  ...
  // --- Hunter Logic ---
  useEffect(() => {
    if (!targetResort || isConfiguring) return; // Wait until configured
    ...
    const payload = {
      ...
      criteria: {
        ...
        lift_proximity_m: parseInt(searchParams.get("proximity") || "500"),
      }
    };
    ...
    huntChalets();
  }, [searchParams, targetResort, country, isDirectOverride, isConfiguring, setChalets, setLastChaletCriteria, lastChaletCriteria, chalets.length, userId]);
  ...
  const finalizeMissionAdd = () => {
    if (selectedChalet) {
      ...
      setSavedToArchive(prev => new Set(prev).add(selectedChalet.name)); 
    }
    closeMissionModal();
  };
  ...
  return (
    <div className="min-h-screen relative font-mono selection:bg-cyan-500 selection:text-white pb-20">
      ...
      <div className="relative z-10 max-w-5xl mx-auto p-6 md:p-10 text-white">
        ...
        {isConfiguring ? (
          <div className="max-w-xl mx-auto mt-12 bg-slate-900/90 backdrop-blur-xl border border-purple-500/50 p-8 rounded-lg shadow-2xl animate-fade-in-down">
            <h2 className="text-2xl font-black text-purple-400 uppercase tracking-widest mb-2 border-b border-slate-800 pb-4">
              PHASE_02A // BASECAMP_PARAMETERS
            </h2>
            <p className="text-sm text-slate-400 mb-8 tracking-widest">
              Target Locked: <span className="text-cyan-400 font-bold">{targetResort}</span>. Configure your deployment parameters before sending AI hunters.
            </p>

            <form onSubmit={handleDeployHunters} className="space-y-6">
              <div>
                <label className="block text-xs font-bold mb-2 uppercase tracking-widest text-slate-300">MAX_BUDGET_PER_NIGHT (EUR)</label>
                <input 
                  type="number" 
                  required
                  value={configForm.budget}
                  onChange={e => setConfigForm({...configForm, budget: e.target.value})}
                  className="w-full bg-slate-950 border border-slate-700 text-white p-4 focus:outline-none focus:border-purple-500 transition-colors" 
                />
              </div>
              <div>
                <label className="block text-xs font-bold mb-2 uppercase tracking-widest text-slate-300">GUEST_COUNT</label>
                <input 
                  type="number" 
                  required
                  min={1}
                  value={configForm.guests}
                  onChange={e => setConfigForm({...configForm, guests: e.target.value})}
                  className="w-full bg-slate-950 border border-slate-700 text-white p-4 focus:outline-none focus:border-purple-500 transition-colors" 
                />
              </div>
              <div>
                <label className="block text-xs font-bold mb-2 uppercase tracking-widest text-slate-300">SPECIAL_REQUIREMENTS (OPTIONAL)</label>
                <input 
                  type="text" 
                  placeholder="e.g. sauna, fireplace, ski-in/ski-out..."
              ...
              <button type="submit" className="w-full bg-purple-600 hover:bg-purple-500 text-white font-black tracking-widest uppercase text-lg py-4 mt-4 transition-all shadow-[0_0_15px_rgba(147,51,234,0.4)]">
                DEPLOY_HUNTERS
              </button>
            </form>
          </div>
        ) : (
          /* --- 2. RESULTS & LOADING SCREEN --- */
          <>
            {loading ? (
              <div className="flex flex-col items-center justify-center pt-32 gap-4 animate-fade-in-down">
                ...
                <div className="text-cyan-500 text-xl font-bold text-center px-4 tracking-widest uppercase">
                  ...
                  <span className="text-slate-600 text-sm font-normal">[ TARGET: {targetResort} | BUDGET: €{searchParams.get("budget")} ]</span>
                </div>
              </div>
            ) : (
              <div className="animate-fade-in-down">
                <div className="border-b border-slate-700 pb-6 mb-10 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                  <div>
                    <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-widest drop-shadow-md flex flex-wrap items-center gap-4">
                      <span className="text-purple-500">_</span> {targetResort}
                    </h1>
                  ...
                  <button 
                    ...
                    className="text-xs text-slate-400 hover:text-purple-400 uppercase tracking-widest border border-slate-800 hover:border-purple-500 px-4 py-2 transition-colors whitespace-nowrap"
                  >
                    [ RECALIBRATE_PARAMS ]
                  </button>
                </div>
                ...
                {chalets.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 gap-8">
                    {chalets.map((chalet, idx) => {
                      ...
                      return (
                        <div key={idx} className="group bg-slate-900/80 backdrop-blur-md border border-slate-700 p-6 md:p-8 rounded-sm flex flex-col md:flex-row gap-8 hover:border-purple-500 transition-all duration-300">
                          ...
                          <div className="flex-1 space-y-4">
                            <div className="flex flex-col md:flex-row md:items-start justify-between border-b border-slate-800 pb-4 gap-4">
                              <div>
                                <h3 className="text-2xl font-bold text-white uppercase tracking-wider group-hover:text-purple-400 transition-colors">
                                  {chalet.name}
                                </h3>
                              ...
                              <div className="flex items-center gap-2 bg-slate-950 border border-purple-900 px-3 py-1 rounded-sm text-purple-400 text-xs tracking-widest shrink-0">
                                [ GEM_SCORE: {chalet.hidden_gem_score}/10 ]
                              </div>
                            </div>

                            <div className="bg-slate-950/50 border-l-2 border-purple-600 p-4">
                            ...
                            <div className="flex flex-wrap gap-3 mt-4">
                              {chalet.price_per_night && (
                                <span className="bg-slate-950 border border-slate-800 px-3 py-2 text-xs text-slate-200 tracking-widest uppercase rounded-sm">
                                  [ PRICE: <span className="text-purple-400 font-bold">€{chalet.price_per_night}</span>/NIGHT ]
                                </span>
                              )}
                              {chalet.distance_to_lift_m && (
                                <span className="bg-slate-950 border border-slate-800 px-3 py-2 text-xs text-slate-200 tracking-widest uppercase rounded-sm">
                                  [ LIFT_DIST: <span className="text-purple-400 font-bold">{chalet.distance_to_lift_m}m</span> ]
                                </span>
                              )}
                            </div>
                          </div>
